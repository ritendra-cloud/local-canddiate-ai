...
def test_chat_sse_persists_new_session(isolated_chat):
    response=isolated_chat.post('/api/chat',json={'message':'Hello?'})
    ...
    detail=isolated_chat.get(f'/api/sessions/{session_id}').json(); assert [m['content'] for m in detail['messages']]==['Hello?','hello world'] and all('system' != m['role'] for m in detail['messages'])
def test_unknown_session_is_safe(isolated_chat):
    response=isolated_chat.post('/api/chat',json={'session_id':'00000000-0000-0000-0000-000000000000','message':'Hello'})
