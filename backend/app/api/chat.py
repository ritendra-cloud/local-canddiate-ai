...
from app.services.chat_service import chat_messages, generation_options, chat_scope, SCOPE_REFUSAL, total_experience_answer, employment_duration_answer, candidate_fact_answer
...
async def chat(payload: ChatRequest, request: Request):
    ...
    try:
        ...
        _,_,may_call=chat_scope(payload.message,prior); profile=__import__('app.services.candidate_service',fromlist=['load_profile']).load_profile(settings.candidate_path); fixed=(candidate_fact_answer(payload.message,profile) or employment_duration_answer(payload.message,profile) or total_experience_answer(payload.message,profile)) if may_call else SCOPE_REFUSAL
        messages=chat_messages(prior,payload.message) if may_call else []
    ...
    async def generate():
        ...
        try:
            ...
            if fixed:
                saved=add_message(db,session,'assistant',fixed); yield event('token',{'content':fixed}); yield event('complete',{'session_id':session.public_id,'message_id':str(saved.id)}); return
